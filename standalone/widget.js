...
function closeModal(event) {
  // Désactivé volontairement : pas de fermeture au clic extérieur (évite les fermetures
  // accidentelles, notamment lors d'une sélection de texte relâchée hors de la modale).
  // Fermeture uniquement via la croix (✕) ou Annuler.
}
...
function openDetailModal(bienId, showManageButtons) {
  ...
  html += '<div class="detail-section-line"></div>';
  html += '<div class="detail-grid-3">';
  // Acquisition Compte Tiers : masqué si vide ; visible si renseigné (anciens biens)
...
function buildFormHtml(bien) {
  ...
  html += '<div class="form-grid">';
  // Acquisition compte tiers : uniquement en modification (conservé pour les anciens
...
async function saveBien(bienId) {
  ...
  try {
    if (bienId) {
      ...
      // Add — on récupère l'id créé puis on bascule en mode édition (évite un doublon au re-clic)
