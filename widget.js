...
// Convertit du HTML (champs Jodit) en texte brut : supprime les balises ET décode les
// entités (&nbsp;, &amp;…). Utilisé à l'export pour ne pas laisser fuir le HTML dans Excel.
function richToPlain(html) {
  if (html === null || html === undefined) return '';
  var s = String(html);
  if (s.indexOf('<') === -1 && s.indexOf('&') === -1) return s; // pas de HTML
  var tmp = document.createElement('div');
  tmp.innerHTML = s;
  var txt = tmp.textContent || tmp.innerText || '';
  return txt.replace(/\xA0/g, " ").trim();
}
// Colonnes stockées en HTML (éditeur riche)
var HTML_RICH_COLS = { Nature_Bien: 1, Observation: 1 };

// Convertit une couleur CSS (rgb(), #hex, #abc, ou nom courant) en [r,g,b] pour jsPDF
...
// Première couleur de texte trouvée dans du HTML (style="color:…" ou <font color>)
...
function openDetailModal(bienId, showManageButtons) {
  ...
  function dv(field) {
    ...
    if (field === 'Nature_Bien' || field === 'Observation') { return val ? String(val) : 'Non spécifié'; }
...
function parseMonthFromDate(dateVal) {
  // Délègue à parseDateFR qui gère TOUS les formats : ISO, dd/mm/yyyy, numéro de série
  // Excel (~40000) ET timestamp Unix en secondes (colonne Date Grist, ex. 1238371200).
  // (L'ancienne logique prenait les timestamps Unix pour des serials Excel → NaN → filtre cassé.)
  var d = parseDateFR(dateVal);
  return (d && !isNaN(d.getTime())) ? (d.getMonth() + 1) : 0;
}
  ...
  for (var r = 0; r < jsonData.length; r++) {
    ...
    for (var h in mapping) {
      ...
      if (IMPORT_DATE_COLS[col] && val !== '' && val !== undefined && val !== null) {
        // parseDateFR gère numéro de série Excel, ISO et dd/mm/yyyy
...
// Remplace l'archive par un instantané des biens actuels (un seul instantané conservé)
async function archiveCurrentBiens() {
  await ensureArchiveTable();
  var B = 50;
  // Vider l'archive existante
...
async function fixExistingDates() {
  ...
  // Repérer les valeurs à corriger
  var updates = [];
  for (var i = 0; i < biens.length; i++) {
    ...
    for (var c = 0; c < DATE_COLS.length; c++) {
      ...
      if (ISO_RE.test(curStr)) continue; // déjà au bon format
